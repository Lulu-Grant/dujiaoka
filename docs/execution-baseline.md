...
最近一轮后台壳已经连续补齐 `goods / order / pay / coupon` 多组低风险批量动作，当前 PHPUnit 基线已推进到 `358 tests / 2053 assertions`，后台壳 smoke 覆盖也持续扩展。
...
1. 商品线：批量压缩商品关键字连续空格，只动 `goods.gd_keywords`
2. 商品线：批量清理购买提示首尾空格，只动 `goods.buy_prompt`
3. 卡密线：批量清理卡密内容首尾空格，只动卡密展示内容字段
4. 卡密线：批量压缩卡密内容连续空格，避开状态和订单关联
5. 订单线：批量压缩订单标题连续空格，只动 `orders.title`
6. 优惠码线：批量压缩优惠码内容连续空格，只动 `coupons.coupon`
7. 文档线：同步 README 与当前基线审计里的动作页清单
8. 兼容层线：审计旧 `/admin/*` 跳转中是否还有可删除入口
9. 支付安全线：补一轮 Stripe webhook 重复通知和金额不一致测试
10. 安全线：复查默认账号、默认密钥、`.env.example` 和安装默认值
