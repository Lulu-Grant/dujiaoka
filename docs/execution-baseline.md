...
7. 每批都确认购买页支付方式、订单查询 tab、首页公告、前台通知和页面渲染不回归
8. 发布候选前每批都跑页面渲染检查、浏览器交互检查、后台壳 smoke 和 `git diff --check`
