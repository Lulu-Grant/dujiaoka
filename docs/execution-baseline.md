...
- 前台体验：v3.1.1 补丁收口期，首页、购买页、订单查询页和确认订单页已完成轻量视觉统一，后续只接受可验证的可读性、性能和移动端转化修正
...
3. 汇总前台首页、购买页、订单查询页和确认订单页的接口边界、物料引用和移动端验收状态
4. 冻结新增功能，只接受阻断级 bug、安全修复、文档口径修正、测试补强和明确性能修正
5. 发布候选前每批都跑全量 PHPUnit、后台壳 smoke、页面渲染检查和 `git diff --check`
