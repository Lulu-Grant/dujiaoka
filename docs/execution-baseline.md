...
1. 文档线：同步 README 与当前基线审计里的动作页清单
2. 兼容层线：审计旧 `/admin/*` 跳转中是否还有可删除入口
3. 支付安全线：补一轮 Stripe webhook 重复通知和金额不一致测试
4. 安全线：复查默认账号、默认密钥、`.env.example` 和安装默认值
5. 升级线：刷新依赖阻塞矩阵并标记仍阻塞项
6. 后台壳线：复查 `goods / order / pay / coupon / carmis` 动作页 smoke 覆盖缺口
7. 支付线：复查 PayPal / Stripe 生命周期文档与代码边界
8. 兼容层线：确认无旧 `app/Admin` 引用残留
9. Release 线：更新第二个测试版 release 候选清单
10. 后台壳线：汇总文本类批量动作的字段边界与回滚说明
