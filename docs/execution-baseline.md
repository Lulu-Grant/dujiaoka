...
最近一轮后台壳已经连续补齐 `goods / order / pay / coupon / carmis` 多组低风险批量动作，并补强 Stripe 支付回调幂等与金额校验护栏；当前 PHPUnit 基线已推进到 `382 tests / 2225 assertions`，后台壳 smoke 覆盖也持续扩展。
...
1. 安全线：复查默认账号、默认密钥、`.env.example` 和安装默认值
2. 升级线：刷新依赖阻塞矩阵并标记仍阻塞项
3. 后台壳线：复查 `goods / order / pay / coupon / carmis` 动作页 smoke 覆盖缺口
4. 支付线：复查 PayPal / Stripe 生命周期文档与代码边界
5. 兼容层线：确认无旧 `app/Admin` 引用残留
6. Release 线：更新第二个测试版 release 候选清单
7. 后台壳线：汇总文本类批量动作的字段边界与回滚说明
8. 兼容层线：补齐旧 `/admin/*` 入口跳转测试缺口后，再评估是否分批移除别名
9. 安全线：补后台批量动作权限与输入边界复查清单
10. 支付安全线：补其他保留支付通道的重复通知幂等回归测试
