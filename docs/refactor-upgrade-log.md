...
### 264. v3.3.0 stable 收口

摘要：

- 新增 [v3.3.0 发布说明](/Users/apple/Documents/dujiaoshuka/docs/releases/v3.3.0.md)，将 `v3.1.1` 前台体验补丁、`v3.2.0` 前台资源拆包和 `v3.3` 支付与安全护栏矩阵收口为稳定版。
- README release 徽章和发布入口同步到 `v3.3.0`，当前测试基线同步为 `OK (417 tests, 4320 assertions)`。
- 当前进度总汇、当前基线审计和执行基线同步为 `v3.3.0 stable` 已收口。
- 本轮不修改下单、订单查询、支付跳转、支付回调、后台动作、前台模板或支付通道实现。

影响范围：

- 影响 release 文档、README、当前进度口径和 release 文档护栏测试。
- 不影响业务代码、数据库结构、公开接口、支付语义或旧 Dcat 最小兼容层。

验证：

- 需执行 `git diff --check`。
- 需执行 release 文档护栏、Dcat 兼容层和当前进度文档 PHPUnit。
- 需执行全量 `vendor/bin/phpunit`。
- 需执行退役通道关键字搜索、真实密钥形态搜索和后台壳 smoke。

下一步：

- 创建并推送 `v3.3.0` tag；后续 `v3.3.x` 只按护栏矩阵补测试或代码护栏，不恢复退役通道，不新增支付 SDK。

### 263. v3.3 支付与安全护栏盘点

摘要：

- 新增 [v3.3 支付与安全护栏矩阵](/Users/apple/Documents/dujiaoshuka/docs/v3.3-payment-security-guardrail-matrix.md)，把保留支付通道、退役通道、安全输入面和升级阻塞面整理为可执行矩阵。
- 支付通道口径固定为官方支付宝、官方微信、易支付和 Epusdt；PayPal、Stripe、Coinbase、Mapay、TokenPay、PayJS、Vpay、Paysapi 等继续保持退役，不恢复、不新增、不进入样例 seed。
- 回调异常路径只做审计和缺口标注，确认签名失败、重复通知、金额不一致、订单不存在、支付方式不匹配和已完成订单重复通知都保持测试护栏维护状态。
- 本轮不修改支付实现、不新增 SDK、不修改 `pay-gateway/{handle}/{payway}/{orderSN}`、回调 URL、订单完成语义、下单或订单查询接口。

影响范围：

- 影响 v3.3 护栏矩阵、当前进度总汇和执行基线口径。
- 不修改后台动作、前台模板、支付通道实现、支付回调、依赖或旧静态文件。

验证：

- 需执行 `git diff --check`。
- 需搜索退役通道关键字，确认只出现在历史日志、退役说明、防回流测试或明确审计记录中。
- 需搜索真实密钥形态，确认 release、安全和示例文档不包含生产 secret 形态。
- 需跑当前进度、Dcat 兼容层、release 文档和支付相关定向 PHPUnit；稳定节点继续跑全量 PHPUnit 和后台壳 smoke。

下一步：

- `v3.3` 第二批只从矩阵中选择测试缺口或护栏缺口执行，优先覆盖上传、模板输入、邮件发送、通知配置和保留支付通道回调异常路径，不进入支付 SDK 扩展或退役通道恢复。

### 262. v3.2 前台资源发布候选验收

摘要：

- 新增 [v3.2.0 前台资源发布候选](/Users/apple/Documents/dujiaoshuka/docs/releases/v3.2.0.md)，记录 `avatar` 前台资源拆包后的最终请求、体积、交互验收和历史文件保留原因。
- 确认 `vendor.min.js`、`app.min.js`、`bootstrap-input-spinner.js`、`icons.min.css`、`bootstrap.min.js` 和 `bootstrap.min.css` 未在前台新页面回流。
- 使用临时前台 fixture 覆盖 `/buy/30`、`/buy/31`、`/buy/32`、`/buy/33`，执行后清理本地测试数据。
- 本轮不修改下单、查询、支付跳转、支付回调、二维码支付轮询或订单详情复制语义。

影响范围：

- 影响发布候选文档、当前进度口径和 `v3.2` 收口记录。
- 不修改前台模板、接口、支付逻辑、后台壳或旧静态文件。

验证：

- Playwright headless 覆盖 `390px / 430px / 768px / 1440px` 的首页、购买页、订单查询页、确认订单页、订单详情页和错误页。
- 验证首页公告 modal、订单查询 tab、购买页提示 modal、支付 tile、toast 校验、订单详情复制按钮和旧重资源不回流。
- 二维码支付页以模板和脚本语义验证，确认二维码容器、二维码插件和轮询跳转逻辑仍存在。

下一步：

- `v3.2` 前台资源拆包进入发布候选完成状态；下一阶段进入 `v3.3` 安全、支付护栏和升级前清障。
...
- 在后台壳卡密线新增 [batch-trim.blade.php](/Users/apple/Documents/dujiaoshuka/resources/views/admin-shell/carmis/batch-trim.blade.php)，支持通过 `/admin/v2/carmis/batch-trim` 先预览命中的卡密，再批量清理卡密内容首尾空格。
- 扩展 [CarmiActionController.php](/Users/apple/Documents/dujiaoshuka/app/Http/Controllers/AdminShell/CarmiActionController.php) 与 [CarmiActionService.php](/Users/apple/Documents/dujiaoshuka/app/Service/CarmiActionService.php)，新增 `batchTrimDefaults`、`batchTrimContext` 和 `trimCarmis`。
- 同步把入口接入卡密列表页头、后台壳资源注册表、后台壳 smoke 和文档动作清单。

影响范围：

- 只更新 `carmis.carmi` 的首尾空格，并只统计实际发生变化的卡密。
- 不触碰销售状态、循环使用标记、商品归属、订单关联、库存扣减和履约链。

验证：

- `./scripts/php74 vendor/bin/phpunit tests/Feature/AdminShellCarmisControllerTest.php tests/Unit/CarmiActionServiceTest.php tests/Unit/AdminShellPageStructureTest.php tests/Unit/AdminShellRouteRegistrarTest.php` 通过，结果为 `OK (16 tests, 88 assertions)`。
- `./scripts/php74 vendor/bin/phpunit` 通过，结果为 `OK (368 tests, 2133 assertions)`。
- `ADMIN_USERNAME=admin-shell-tester ADMIN_PASSWORD=secret123 ./scripts/smoke-admin-shell` 通过。

下一步：

- 按执行基线继续推进 `carmis` 线，优先补卡密内容连续空格压缩。

### 191. 商品管理接入批量清理购买提示空格页

摘要：

- 在后台壳商品线新增 [batch-buy-prompt-trim.blade.php](/Users/apple/Documents/dujiaoshuka/resources/views/admin-shell/goods/batch-buy-prompt-trim.blade.php)，支持通过 `/admin/v2/goods/create?mode=batch-buy-prompt-trim` 先预览命中的商品，再批量清理购买提示首尾空格。
- 扩展 [GoodsActionController.php](/Users/apple/Documents/dujiaoshuka/app/Http/Controllers/AdminShell/GoodsActionController.php) 与 [GoodsActionService.php](/Users/apple/Documents/dujiaoshuka/app/Service/GoodsActionService.php)，新增 `batchBuyPromptTrimDefaults` 和 `trimBuyPrompts`。
- 同步把入口接入商品列表页头、后台壳 smoke 和文档动作清单。

影响范围：

- 只更新 `goods.buy_prompt` 的首尾空格，并只统计实际发生变化的商品。
- 不触碰商品价格、库存、分类、商品类型、销量、排序、启用状态、商品简介、商品关键字、长商品说明、履约配置和优惠码关联。

验证：

- `./scripts/php74 vendor/bin/phpunit tests/Feature/AdminShellGoodsControllerTest.php tests/Unit/GoodsActionServiceTest.php tests/Unit/AdminShellPageStructureTest.php` 通过，结果为 `OK (36 tests, 283 assertions)`。
- `./scripts/php74 vendor/bin/phpunit` 通过，结果为 `OK (364 tests, 2106 assertions)`。
- `ADMIN_USERNAME=admin-shell-tester ADMIN_PASSWORD=secret123 ./scripts/smoke-admin-shell` 通过。

下一步：

- 按执行基线切入 `carmis` 线，优先补卡密内容首尾空格清理。
...
- 在后台壳商品线新增 [batch-description-trim.blade.php](/Users/apple/Documents/dujiaoshuka/resources/views/admin-shell/goods/batch-description-trim.blade.php)，支持通过 `/admin/v2/goods/create?mode=batch-description-trim` 先预览命中的商品，再批量清理商品简介首尾空格。
- 扩展 [GoodsActionController.php](/Users/apple/Documents/dujiaoshuka/app/Http/Controllers/AdminShell/GoodsActionController.php) 与 [GoodsActionService.php](/Users/apple/Documents/dujiaoshuka/app/Service/GoodsActionService.php)，新增 `batchDescriptionTrimDefaults`、`batchDescriptionTrimContext` 和 `trimGdDescriptions`。
- 同步把入口接入商品列表页头、后台壳 smoke 和文档动作清单。

影响范围：

- 只更新 `goods.gd_description` 的首尾空格，并只统计实际发生变化的商品。
- 不触碰商品价格、库存、分类、商品类型、销量、排序、启用状态、购买提示、长商品说明、履约配置和优惠码关联。

验证：

- `./scripts/php74 vendor/bin/phpunit tests/Feature/AdminShellGoodsControllerTest.php tests/Unit/GoodsActionServiceTest.php tests/Unit/AdminShellPageStructureTest.php` 通过，结果为 `OK (32 tests, 238 assertions)`。
- `./scripts/php74 vendor/bin/phpunit` 通过，结果为 `OK (358 tests, 2053 assertions)`。
- `ADMIN_USERNAME=admin-shell-tester ADMIN_PASSWORD=secret123 ./scripts/smoke-admin-shell` 通过。

下一步：

- 按执行基线继续推进 `goods` 线，优先补 `gd_keywords` 连续空格压缩或 `buy_prompt` 首尾空格清理。
...
- 在后台壳订单线新增 [batch-title-trim.blade.php](/Users/apple/Documents/dujiaoshuka/resources/views/admin-shell/order/batch-title-trim.blade.php)，支持按订单 ID 预览并批量清理订单标题首尾空格。
- 扩展 [OrderActionService.php](/Users/apple/Documents/dujiaoshuka/app/Service/OrderActionService.php) 与 [OrderActionController.php](/Users/apple/Documents/dujiaoshuka/app/Http/Controllers/AdminShell/OrderActionController.php)，新增 `batchTitleTrimDefaults`、`trimTitles` 和对应 GET/POST 动作。
- 同步把 `/admin/v2/order/batch-title-trim` 注册到后台壳资源注册表、订单列表头部动作和 smoke 检查。

影响范围：

- 只更新 `orders.title` 的首尾空格，并只统计实际发生变化的订单
- 不触碰订单状态机、支付、履约、通知链、查询密码、订单类型和交易信息
...
- 继续沿 order/carmis/pay 线补齐只改展示或人工维护辅助字段的低风险批量动作，保持后台壳迁移节奏稳定

### 185. 优惠码接入批量清理内容空格页

摘要：

- 在后台壳优惠码线新增 [batch-code-trim.blade.php](/Users/apple/Documents/dujiaoshuka/resources/views/admin-shell/coupon/batch-code-trim.blade.php)，支持按优惠码 ID 预览并批量清理优惠码内容首尾空格。
- 扩展 [CouponActionService.php](/Users/apple/Documents/dujiaoshuka/app/Service/CouponActionService.php) 与 [CouponActionController.php](/Users/apple/Documents/dujiaoshuka/app/Http/Controllers/AdminShell/CouponActionController.php)，新增 `batchCodeTrimDefaults`、`trimCodes` 和对应 GET/POST 动作。
- 同步把 `/admin/v2/coupon/batch-code-trim` 注册到后台壳资源注册表、优惠码列表头部动作和 smoke 检查。

影响范围：

- 只更新 `coupons.coupon` 的首尾空格，并只统计实际发生变化的优惠码
- 不触碰折扣、可用次数、启停状态、使用状态和商品关联
...
- 继续沿 coupon/carmis/order 线补齐只改展示或运营辅助字段的低风险批量动作，保持后台壳扩容小步稳定
...
- 在后台壳支付通道线新增 [batch-name-trim.blade.php](/Users/apple/Documents/dujiaoshuka/resources/views/admin-shell/pay/batch-name-trim.blade.php)，支持按支付通道 ID 预览并批量清理支付名称首尾空格。
- 扩展 [PayActionService.php](/Users/apple/Documents/dujiaoshuka/app/Service/PayActionService.php) 与 [PayActionController.php](/Users/apple/Documents/dujiaoshuka/app/Http/Controllers/AdminShell/PayActionController.php)，新增 `batchNameTrimDefaults`、`trimNames` 和对应 GET/POST 动作。
- 同步把 `/admin/v2/pay/batch-name-trim` 注册到后台壳资源注册表、支付列表头部动作和 smoke 检查。

影响范围：

- 只更新 `pays.pay_name` 的首尾空格，并只统计实际发生变化的通道
...
- 继续沿 pay 或 carmis 线补齐低风险展示/运营辅助动作，保持后台壳迁移小步稳定
...
### 169. 优惠码管理接入批量重生成优惠码内容页

摘要：

- 优惠码管理现在新增了 [batch-code.blade.php](/Users/apple/Documents/dujiaoshuka/resources/views/admin-shell/coupon/batch-code.blade.php)，可以通过 `/admin/v2/coupon/batch-code` 先预览匹配优惠码，再统一重生成新的优惠码内容。
- [CouponActionController.php](/Users/apple/Documents/dujiaoshuka/app/Http/Controllers/AdminShell/CouponActionController.php) 新增了 `batch-code` 模式下的页面渲染和提交处理，把这条低风险维护动作接进现有优惠码动作控制器。
- [CouponActionService.php](/Users/apple/Documents/dujiaoshuka/app/Service/CouponActionService.php) 新增了批量换码默认值和基于前缀/长度的唯一优惠码重生成逻辑，继续把写入边界压在服务层。
- [AdminShellCouponPageService.php](/Users/apple/Documents/dujiaoshuka/app/Service/AdminShellCouponPageService.php) 已在优惠码概览页头补上“批量重生成优惠码”入口。
...
- 优惠码壳页从“启停 + 使用状态 + 折扣 + 可用次数 + 导出 + 批量生成”继续推进到了“又一个真实低风险维护动作”。
- 后台壳在优惠码人工维护线上的能力更完整了，适合统一更新活动期优惠码前缀、重整测试码样式或人工换码，而不会误动折扣、可用次数、启用状态、使用状态和关联商品。
- 当前这条动作只更新 `coupon`，不改 `discount`、`ret`、`is_open`、`is_use` 和 `goods` 关联。

验证：

- `./scripts/php74 vendor/bin/phpunit tests/Feature/AdminShellCouponControllerTest.php tests/Unit/AdminShellPageStructureTest.php tests/Unit/AdminShellRouteRegistrarTest.php tests/Unit/CouponActionServiceTest.php` 通过，结果为 `OK (20 tests, 144 assertions)`
- `./scripts/php74 vendor/bin/phpunit` 通过，结果为 `OK (302 tests, 1656 assertions)`
- `ADMIN_USERNAME=admin-shell-tester ADMIN_PASSWORD=secret123 ./scripts/smoke-admin-shell` 通过
...
- 商品管理现在新增了 [batch-description.blade.php](/Users/apple/Documents/dujiaoshuka/resources/views/admin-shell/goods/batch-description.blade.php)，可以通过 `/admin/v2/goods/create?mode=batch-description` 先预览命中的商品，再统一调整商品说明。
- [GoodsActionController.php](/Users/apple/Documents/dujiaoshuka/app/Http/Controllers/AdminShell/GoodsActionController.php) 新增了 `batch-description` 模式下的页面渲染和提交处理，把这条低风险批量动作接进现有商品动作控制器。
- [GoodsActionService.php](/Users/apple/Documents/dujiaoshuka/app/Service/GoodsActionService.php) 新增了批量商品说明默认值、匹配预览上下文和 `description` 批量更新逻辑，继续把写入边界压在服务层。
- [AdminShellGoodsPageService.php](/Users/apple/Documents/dujiaoshuka/app/Service/AdminShellGoodsPageService.php) 已在商品概览页头补上“批量设置商品说明”入口。
- [AdminShellGoodsControllerTest.php](/Users/apple/Documents/dujiaoshuka/tests/Feature/AdminShellGoodsControllerTest.php)、[GoodsActionServiceTest.php](/Users/apple/Documents/dujiaoshuka/tests/Unit/GoodsActionServiceTest.php)、[AdminShellPageStructureTest.php](/Users/apple/Documents/dujiaoshuka/tests/Unit/AdminShellPageStructureTest.php) 和 [tests/Browser/admin-shell-smoke.sh](/Users/apple/Documents/dujiaoshuka/tests/Browser/admin-shell-smoke.sh) 已把这条动作的展示、页头合同、服务默认值和页面巡检护栏补齐。

影响范围：

- 商品壳页从“启停 + 切换分类 + 限购维护 + 销量维护 + 排序维护 + 购买提示维护 + 导出”继续推进到了“又一个真实低风险批量动作”。
- 后台壳在商品运营文案维护线上的能力更完整了，适合活动期统一补充商品说明、售后须知和交付细节，而不会误动价格、库存、分类、商品类型、销量、排序和启用状态。
- 当前这条动作只更新 `description`，不改 `actual_price`、`in_stock`、`group_id`、`type`、`sales_volume`、`ord`、`buy_prompt` 和 `is_open`。
...
- 优惠码管理现在新增了 [batch-code-suffix.blade.php](/Users/apple/Documents/dujiaoshuka/resources/views/admin-shell/coupon/batch-code-suffix.blade.php)，可以通过 `/admin/v2/coupon/batch-code-suffix` 先预览命中的优惠码，再统一给优惠码内容添加后缀。
- [CouponActionController.php](/Users/apple/Documents/dujiaoshuka/app/Http/Controllers/AdminShell/CouponActionController.php) 新增了 `batch-code-suffix` 的页面渲染和提交处理，把这条低风险批量动作接进现有优惠码动作控制器。
- [CouponActionService.php](/Users/apple/Documents/dujiaoshuka/app/Service/CouponActionService.php) 新增了批量后缀默认值和唯一性保护逻辑；如果追加后缀后发生优惠码重复，会自动追加编号。
- [AdminShellCouponPageService.php](/Users/apple/Documents/dujiaoshuka/app/Service/AdminShellCouponPageService.php) 已在优惠码概览页头补上“批量添加优惠码后缀”入口。
- [AdminShellResourceRegistry.php](/Users/apple/Documents/dujiaoshuka/app/Service/AdminShellResourceRegistry.php)、[AdminShellCouponControllerTest.php](/Users/apple/Documents/dujiaoshuka/tests/Feature/AdminShellCouponControllerTest.php)、[CouponActionServiceTest.php](/Users/apple/Documents/dujiaoshuka/tests/Unit/CouponActionServiceTest.php)、[AdminShellPageStructureTest.php](/Users/apple/Documents/dujiaoshuka/tests/Unit/AdminShellPageStructureTest.php)、[AdminShellRouteRegistrarTest.php](/Users/apple/Documents/dujiaoshuka/tests/Unit/AdminShellRouteRegistrarTest.php) 和 [tests/Browser/admin-shell-smoke.sh](/Users/apple/Documents/dujiaoshuka/tests/Browser/admin-shell-smoke.sh) 已把这条动作的路由、展示、默认值、页头合同和页面巡检护栏补齐。

影响范围：

- 优惠码壳页从“启停 + 使用状态 + 折扣 + 次数 + 重生成 + 添加前缀 + 导出”继续推进到了“又一条真实低风险批量动作”。
- 后台壳在优惠码运营维护线上的能力更完整了，适合活动结束标记、渠道批次归档或临时测试尾标。
- 当前这条动作只更新 `coupon`，不改 `discount`、`ret`、`is_open`、`is_use` 和商品关联。
...
- 订单管理现在新增了 [batch-title.blade.php](/Users/apple/Documents/dujiaoshuka/resources/views/admin-shell/order/batch-title.blade.php)，可以通过 `/admin/v2/order/batch-title` 先预览命中的订单，再统一调整订单标题。
- [OrderActionController.php](/Users/apple/Documents/dujiaoshuka/app/Http/Controllers/AdminShell/OrderActionController.php) 新增了 `batch-title` 的页面渲染和提交处理，把这条低风险批量动作接进现有订单动作控制器。
- [OrderActionService.php](/Users/apple/Documents/dujiaoshuka/app/Service/OrderActionService.php) 新增了批量标题默认值和 `title` 批量更新逻辑，继续把写入边界压在服务层，并保持无事件写入。
- [AdminShellOrderPageService.php](/Users/apple/Documents/dujiaoshuka/app/Service/AdminShellOrderPageService.php) 已在订单概览页头补上“批量设置订单标题”入口。
- [AdminShellOrderControllerTest.php](/Users/apple/Documents/dujiaoshuka/tests/Feature/AdminShellOrderControllerTest.php)、[OrderActionServiceTest.php](/Users/apple/Documents/dujiaoshuka/tests/Unit/OrderActionServiceTest.php)、[AdminShellPageStructureTest.php](/Users/apple/Documents/dujiaoshuka/tests/Unit/AdminShellPageStructureTest.php)、[AdminShellRouteRegistrarTest.php](/Users/apple/Documents/dujiaoshuka/tests/Unit/AdminShellRouteRegistrarTest.php) 和 [tests/Browser/admin-shell-smoke.sh](/Users/apple/Documents/dujiaoshuka/tests/Browser/admin-shell-smoke.sh) 已把这条动作的展示、默认值、路由注册和页面巡检护栏补齐。

影响范围：

- 订单壳页从“状态 + 类型 + 附加信息 + 查询密码重置 + 导出”继续推进到了“又一条真实低风险批量动作”。
- 后台壳在订单人工维护线上的能力更完整了，适合统一补充活动标签、人工复核标记和历史标题整理，而不会误动状态机、支付、履约和通知链。
- 当前这条动作只更新 `title`，不改 `status`、`type`、`info`、`search_pwd`、`trade_no` 和支付相关字段。
