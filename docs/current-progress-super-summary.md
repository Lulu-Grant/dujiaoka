...
当前仓库已经从“停更遗留项目抢救”推进到了“有稳定基线、后台替换主线、安装现代化完成、前台体验补丁收口、升级前清障在路上的持续维护项目”。
...
- `v3.2` 前台资源拆包已完成三批，`avatar` 前台已停止加载 `vendor.min.js`、`app.min.js`、`bootstrap-input-spinner.js`、`icons.min.css` 和 `bootstrap.min.js`，并由 `hyper.js` 接管轻量通知、modal 和 tab 兼容逻辑
...
1. 维护 v3.1.1 前台体验补丁、测试记录和接口冻结边界
...
7. 前台性能清理进入 `v3.2`：已停载 `vendor.min.js / app.min.js / bootstrap-input-spinner.js / icons.min.css / bootstrap.min.js`，下一批优先评估 `Bootstrap CSS` 的真实依赖和替换边界
