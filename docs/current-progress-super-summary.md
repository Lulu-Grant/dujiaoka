...
- `v3.2` 前台资源第一批拆包已开始，`avatar` 前台已停止加载 `vendor.min.js`、`app.min.js` 和 `bootstrap-input-spinner.js`，并由 `hyper.js` 接管轻量通知、modal 和 tab 兼容逻辑
...
7. 前台性能清理进入 `v3.2`：第一批已停载 `vendor.min.js / app.min.js / bootstrap-input-spinner.js`，下一批优先评估 `icons.min.css / fonts / Bootstrap CSS`
