...
- `v3.2` 前台资源拆包已完成两批，`avatar` 前台已停止加载 `vendor.min.js`、`app.min.js`、`bootstrap-input-spinner.js` 和 `icons.min.css`，并由 `hyper.js` 接管轻量通知、modal 和 tab 兼容逻辑
...
7. 前台性能清理进入 `v3.2`：已停载 `vendor.min.js / app.min.js / bootstrap-input-spinner.js / icons.min.css`，下一批优先评估 `Bootstrap CSS/JS` 的真实依赖和替换边界
