...
- v3.2 前台资源拆包发布候选已完成
- v3.3 支付与安全护栏盘点已启动，同时升级前清障保持 3.2 / 4.0 实验准备期边界
...
- 已完成第一大轮，且进入可维护状态
- `v3.3` 第一批已进入护栏盘点，不改支付实现、不新增 SDK、不恢复退役通道
...
- “结构化治理后期，后台替换 v3.1 stable 冻结期，v3.2 前台资源发布候选完成，v3.3 支付与安全护栏盘点启动，升级前清障保持 3.2 / 4.0 实验准备期”
...
1. 维护 v3.1.0 / v3.2.0 文档、测试数字和冻结边界
