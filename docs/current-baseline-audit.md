...
- 已经有清单、状态表、依赖阻塞矩阵、CI 护栏和升级实验命令，当前按 v3.1.0-beta.1 要求维护

还没完成的点：

- 远端 CI 与最终发版分支复核
- v3.1.0-beta.1 发布说明和遗留边界持续维护
- PHP / Laravel 后续升级实验单独分支验证

当前 v3.1.0-beta.1 收口口径：
...
- 升级前清障以依赖阻塞矩阵和实验分支验证清单为准，不直接跳 Laravel 10+，不混入 v3.1.0-beta.1 收口。
...
- “结构化治理后期，后台替换 v3.1 beta 冻结期，升级前清障进入 3.2 / 4.0 实验准备期”
...
1. 维护 v3.1.0-beta.1 文档、测试数字和冻结边界
2. 复核远端 CI 与最终发版分支
