...
- 支付样例种子已停止灌入退役通道，避免新环境继续引入历史网关
- 后台支付通道管理页已开始展示生命周期状态，便于识别维护中、遗留待替换与已退役通道
- 支付层剩余最高风险集中在 `StripeController` 与 PayPal 历史 SDK 本体

## 下一步默认动作

1. 继续补支付关键异常路径测试：重复通知、签名失败、金额不一致、已完成订单重复推进、异常路径不触发履约。
2. 制定 PayPal 历史 SDK 的保留 / 替换 / 退场方案，并保持旧 SDK 只通过 `PaypalSdkService` 和接口边界访问。
3. 在 Stripe `20.x` 基线上继续稳定 `StripePaymentService`、`StripeCheckoutService` 和 `StripeSdkService`，不扩大控制器直连 SDK 面。
4. 视情况引入更显式的支付网关适配接口。
...
- 已退役通道 `Paysapi / Vpay / Payjs` 不重新进入路由、样例种子或后台新增功能。
