...
class MapayControllerTest extends TestCase
{
    ...
    {
        $order = $this->createMapayOrder('MAPAY-FAIL-001');

        $request = Request::create('/pay/mapay/notify_url', 'POST', [
        ...
        $response = app(MapayController::class)->notifyUrl($request);
    ...
    {
        $order = $this->createMapayOrder('MAPAY-SUCCESS-001');
        $payGateway = Pay::query()->findOrFail($order->pay_id);

        $payload = [
            'pay_id' => $order->order_sn,
            'money' => '10',
            'pay_no' => 'PAY-NO-SUCCESS',
        ];
        $payload['sign'] = md5(signquery_string($payload) . $payGateway->merchant_pem);

        $request = Request::create('/pay/mapay/notify_url', 'POST', $payload);
        ...
        $this->assertSame($order->order_sn, $order->trade_no);
    }

    private function createMapayOrder(string $orderSn): Order
    {
        $group = GoodsGroup::query()->create([
            'gp_name' => 'Mapay Group ' . $orderSn,
        ...
        $goods = Goods::query()->create([
            'group_id' => $group->id,
            'gd_name' => 'Mapay Product ' . $orderSn,
            'gd_description' => 'Mapay Product Description',
            'gd_keywords' => 'mapay,product',
            'actual_price' => 10.00,
            'in_stock' => 10,
            'sales_volume' => 0,
