...
class VpayControllerTest extends TestCase
{
    ...
    {
        $order = $this->createVpayOrder('VPAY-FAIL-001');

        $request = Request::create('/pay/vpay/notify_url', 'GET', [
            'payId' => 'PAY-ID-FAIL',
        ...
        $response = app(VpayController::class)->notifyUrl($request);
    ...
    {
        $order = $this->createVpayOrder('VPAY-SUCCESS-001');
        $payGateway = Pay::query()->findOrFail($order->pay_id);

        $payload = [
            'payId' => 'PAY-ID-SUCCESS',
            'param' => $order->order_sn,
            'type' => '1',
            'price' => '10',
            'reallyPrice' => '10',
        ...
        $payload['sign'] = md5(
            ...
            . $payload['price']
            . $payload['reallyPrice']
            . $payGateway->merchant_id
        );

        $request = Request::create('/pay/vpay/notify_url', 'GET', $payload);
    ...
    private function createVpayOrder(string $orderSn): Order
    {
        $group = GoodsGroup::query()->create([
            'gp_name' => 'Vpay Group ' . $orderSn,
        ...
        $goods = Goods::query()->create([
            'group_id' => $group->id,
            'gd_name' => 'Vpay Product ' . $orderSn,
            'gd_description' => 'Vpay Product Description',
            'gd_keywords' => 'vpay,product',
            'actual_price' => 10.00,
            'in_stock' => 10,
            'sales_volume' => 0,
        ...
        $pay = Pay::query()->create([
            'pay_name' => 'Vpay ' . $orderSn,
            'pay_check' => 'vpay-' . strtolower($orderSn),
            'pay_method' => Pay::METHOD_JUMP,
            'pay_client' => Pay::PAY_CLIENT_PC,
            'merchant_id' => 'vpay-secret',
