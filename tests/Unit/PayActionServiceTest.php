...
class PayActionServiceTest extends TestCase
{
    public function test_create_context_for_copying_preserves_safe_summary(): void
    {
        ...
        $source = new Pay();
        $source->pay_name = 'Epusdt 样板';
        $source->pay_check = 'epusdt';

        $context = $service->createContext($source);

        $this->assertSame('复制支付通道：Epusdt 样板', $context['summaryTitle']);
        $this->assertStringContainsString('复制来源', $context['summaryItems'][0]['label']);
        $this->assertStringContainsString('Epusdt 样板', $context['summaryItems'][0]['value']);
    ...
    public function test_create_sections_for_copying_keep_secrets_blank(): void
    {
        ...
        $source->pay_handleroute = '/pay/epusdt';
        ...
        $this->assertSame('Epusdt 样板（副本）', $fieldMap['pay_name']['value']);
        ...
        $this->assertSame('/pay/epusdt', $fieldMap['pay_handleroute']['value']);
    }
    ...
    public function test_create_defaults_for_copying_keep_secrets_blank(): void
    {
        ...
        $this->assertSame('Epusdt 样板（副本）', $defaults['pay_name']);
        ...
        $this->assertSame('/pay/epusdt', $defaults['pay_handleroute']);
    }
    ...
    public function test_batch_name_trim_defaults_only_require_ids_for_safe_review(): void
    {
        $service = $this->app->make(PayActionService::class);

        $defaults = $service->batchNameTrimDefaults([601, 602]);
