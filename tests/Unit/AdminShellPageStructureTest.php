...
class AdminShellPageStructureTest extends TestCase
{
    public function test_goods_group_page_service_builds_table_and_detail_items()
    {
        ...
        $this->assertSame('新建商品分类', $header['actions'][0]['label']);
    ...
    public function test_goods_page_service_builds_table_and_detail_items()
    {
        ...
        $this->assertSame('批量切换分类', $header['actions'][2]['label']);
        ...
        $this->assertSame('批量清理购买提示空格', $header['actions'][6]['label']);
        $this->assertSame('批量设置商品说明', $header['actions'][7]['label']);
        $this->assertSame('批量清理简介空格', $header['actions'][8]['label']);
        $this->assertSame('批量设置关键字', $header['actions'][9]['label']);
        $this->assertSame('批量添加关键字后缀', $header['actions'][10]['label']);
        $this->assertSame('批量清理关键字空格', $header['actions'][11]['label']);
        $this->assertSame('批量压缩关键字空格', $header['actions'][12]['label']);
        $this->assertSame('导出文本', $header['actions'][13]['label']);
        $this->assertSame('导出 CSV', $header['actions'][14]['label']);
        $this->assertStringContainsString('export=text', $header['actions'][13]['href']);
        $this->assertStringContainsString('export=csv', $header['actions'][14]['href']);
    ...
    public function test_email_template_page_service_builds_table_and_detail_items()
    {
        ...
        $this->assertSame('新建邮件模板', $header['actions'][0]['label']);
        $this->assertSame('预览样例模板', $header['actions'][1]['label']);
        $this->assertSame('导出当前筛选摘要', $header['actions'][2]['label']);
        $this->assertStringContainsString('export=summary', $header['actions'][2]['href']);
        $this->assertStringContainsString('tpl_name=%E5%8F%91%E8%B4%A7', $header['actions'][2]['href']);
        $this->assertStringContainsString('tpl_token=deliver_notice', $header['actions'][2]['href']);
    ...
    public function test_pay_page_service_builds_table_and_detail_items()
    {
        ...
        $this->assertSame('批量启停通道', $header['actions'][0]['label']);
        $this->assertSame('批量切换场景', $header['actions'][1]['label']);
        $this->assertSame('批量切换方式', $header['actions'][2]['label']);
        $this->assertSame('批量设置名称', $header['actions'][3]['label']);
        $this->assertSame('批量添加名称前缀', $header['actions'][4]['label']);
        $this->assertSame('批量添加名称后缀', $header['actions'][5]['label']);
        $this->assertSame('批量替换名称片段', $header['actions'][6]['label']);
        $this->assertSame('批量清理名称空格', $header['actions'][7]['label']);
        $this->assertSame('批量压缩名称空格', $header['actions'][8]['label']);
    ...
    public function test_order_page_service_builds_table_and_detail_items()
    {
        ...
        $this->assertSame('批量更新订单状态', $header['actions'][0]['label']);
        $this->assertSame('批量设置订单类型', $header['actions'][1]['label']);
        $this->assertSame('批量设置附加信息', $header['actions'][2]['label']);
        $this->assertSame('批量设置订单标题', $header['actions'][3]['label']);
        $this->assertSame('批量添加标题前缀', $header['actions'][4]['label']);
        $this->assertSame('批量添加标题后缀', $header['actions'][5]['label']);
        $this->assertSame('批量清理标题空格', $header['actions'][6]['label']);
        $this->assertSame('批量压缩标题空格', $header['actions'][7]['label']);
        $this->assertSame('批量重置查询密码', $header['actions'][8]['label']);
    ...
    public function test_coupon_page_service_builds_table_and_detail_items()
    {
        ...
        $this->assertSame('导出优惠码文本', $header['actions'][0]['label']);
        $this->assertSame('导出优惠码 CSV', $header['actions'][1]['label']);
        $this->assertSame('批量启停优惠码', $header['actions'][2]['label']);
        $this->assertSame('批量设置使用状态', $header['actions'][3]['label']);
        $this->assertSame('批量设置折扣', $header['actions'][4]['label']);
        $this->assertSame('批量设置可用次数', $header['actions'][5]['label']);
        $this->assertSame('批量重生成优惠码', $header['actions'][6]['label']);
        $this->assertSame('批量添加优惠码前缀', $header['actions'][7]['label']);
        $this->assertSame('批量添加优惠码后缀', $header['actions'][8]['label']);
        $this->assertSame('批量替换优惠码片段', $header['actions'][9]['label']);
        $this->assertSame('批量清理优惠码空格', $header['actions'][10]['label']);
        $this->assertSame('批量压缩优惠码空格', $header['actions'][11]['label']);
    ...
    public function test_carmis_page_service_builds_table_and_detail_items()
    {
        ...
        $this->assertSame('新建卡密', $header['actions'][0]['label']);
        $this->assertSame('导入卡密', $header['actions'][1]['label']);
        $this->assertSame('批量设置循环使用', $header['actions'][2]['label']);
        $this->assertSame('批量清理内容空格', $header['actions'][3]['label']);
        $this->assertSame('批量压缩内容空格', $header['actions'][4]['label']);
        $this->assertSame('批量替换内容片段', $header['actions'][5]['label']);
        $this->assertSame('批量追加内容后缀', $header['actions'][6]['label']);
    ...
    public function test_system_setting_page_service_builds_table_and_detail_items()
    {
        ...
        $this->assertSame('编辑订单行为配置', $header['actions'][3]['label']);
