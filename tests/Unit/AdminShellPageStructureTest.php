...
class AdminShellPageStructureTest extends TestCase
{
    ...
    public function test_pay_page_service_builds_table_and_detail_items()
    {
        ...
        $this->assertSame('新建支付通道', $header['actions'][4]['label']);
        $this->assertSame('导出结构化 CSV', $header['actions'][5]['label']);
        $this->assertSame('导出当前筛选', $header['actions'][6]['label']);
        $this->assertStringContainsString('export=csv', $header['actions'][5]['href']);
        $this->assertStringContainsString('export=txt', $header['actions'][6]['href']);
