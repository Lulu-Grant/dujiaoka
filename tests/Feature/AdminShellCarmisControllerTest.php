...
class AdminShellCarmisControllerTest extends TestCase
{
    ...
    public function test_batch_trim_page_renders_matching_preview(): void
    {
        $this->seedCarmiFixture(95001, '测试商品卡密 A', 'CARD-AAA-001');
        $this->seedCarmiFixture(95002, '测试商品卡密 B', 'CARD-BBB-002');

        $response = $this->actingAs($this->makeAdmin(), 'admin')
            ->get('/admin/v2/carmis/batch-trim?ids=95001,95002,95999');

        $response->assertOk();
        $response->assertSee('批量清理卡密内容空格');
        $response->assertSee('测试商品卡密 A');
        $response->assertSee('测试商品卡密 B');
        $response->assertSee('CARD-AAA-001');
        $response->assertSee('95999');
    ...
    public function test_batch_trim_page_can_trim_carmis_without_touching_status_or_relations(): void
    {
        $this->seedCarmiFixture(95001, '测试商品卡密 A', '  CARD-AAA-001  ');
        ...
        $response = $this->actingAs($this->makeAdmin(), 'admin')
            ->post('/admin/v2/carmis/batch-trim', [
                'ids_text' => "95001\n95002,95999",
            ]);

        $response->assertRedirect('/admin/v2/carmis/batch-trim?ids=95001,95002,95999');
        $response->assertSessionHas('status', '已批量清理 1 条卡密的内容空格');
