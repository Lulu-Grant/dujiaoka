...
@section('content')
    <section class="admin-shell-stack">
        ...
        <section class="admin-shell-card">
            <div class="admin-shell-card__body">
                ...
                <form method="POST" action="{{ $formAction }}" class="admin-shell-form admin-shell-stack">
                    @csrf
                    <input type="hidden" name="mode" value="batch-keywords-suffix">
                    ...
                    <div class="admin-shell-form__field">
                        <label class="admin-shell-form__label" for="keywords_suffix">目标关键字后缀</label>
                        <input id="keywords_suffix" type="text" name="keywords_suffix" class="admin-shell-form__input" maxlength="100" placeholder="例如：,春季活动" value="{{ old('keywords_suffix', $defaults['keywords_suffix']) }}">
                        <p class="admin-shell-form__hint">提交后会把当前商品关键字改成“原关键字 + 后缀”，适合追加活动标签。</p>
                        @error('keywords_suffix')
        ...
        <section class="admin-shell-card">
            <div class="admin-shell-card__header">
                <h2>命中商品预览</h2>
                <p class="admin-shell-card__description">先看一眼商品名称、类型、当前状态和当前关键字，再执行后缀追加，避免粘错 ID。</p>
            </div>
            <div class="admin-shell-card__body">
