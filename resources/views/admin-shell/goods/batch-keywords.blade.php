...
@section('content')
    <section class="admin-shell-stack">
        ...
        <section class="admin-shell-card">
            <div class="admin-shell-card__body">
                ...
                <form method="POST" action="{{ $formAction }}" class="admin-shell-form admin-shell-stack">
                    @csrf
                    <input type="hidden" name="mode" value="batch-keywords">
                    ...
                    <div class="admin-shell-form__field">
                        <label class="admin-shell-form__label" for="gd_keywords">目标商品关键字</label>
                        <input id="gd_keywords" type="text" name="gd_keywords" class="admin-shell-form__input" maxlength="255" placeholder="例如：西瓜版,自动发货,活动专区" value="{{ old('gd_keywords', $defaults['gd_keywords']) }}">
                        <p class="admin-shell-form__hint">这里会统一覆盖命中商品的关键字，适合活动检索词、后台识别标签和 SEO 关键字统一维护。</p>
                        @error('gd_keywords')
        ...
        <section class="admin-shell-card">
            <div class="admin-shell-card__header">
                <h2>命中商品预览</h2>
                <p class="admin-shell-card__description">先看一眼商品名称、类型、当前状态和当前关键字，再执行批量更新，避免粘错 ID。</p>
            </div>
            <div class="admin-shell-card__body">
