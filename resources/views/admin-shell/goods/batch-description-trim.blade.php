...
@section('content')
    <section class="admin-shell-stack">
        ...
        <section class="admin-shell-card">
            <div class="admin-shell-card__body">
                ...
                <form method="POST" action="{{ $formAction }}" class="admin-shell-form admin-shell-stack">
                    @csrf
                    <input type="hidden" name="mode" value="batch-description-trim">
                    ...
                    <div class="admin-shell-form__field">
                        ...
                        <p class="admin-shell-form__hint">支持换行、逗号和空格混输。保存时会自动去重，只清理命中商品简介首尾空格。</p>
                    </div>
        ...
        <section class="admin-shell-card">
            <div class="admin-shell-card__header">
                <h2>命中商品预览</h2>
                <p class="admin-shell-card__description">先看一眼商品名称、类型、当前状态和当前简介，再执行空格清理，避免粘错 ID。</p>
            </div>
            <div class="admin-shell-card__body">
                    ...
                    <table class="admin-shell-table">
                        ...
                        @forelse($context['items'] as $item)
                            <tr>
                                ...
                                <td>{{ $item['type'] }}</td>
                                <td>{{ $item['status'] }}</td>
                                <td>{{ $item['gd_description'] === '' ? '未设置' : $item['gd_description'] }}</td>
                            </tr>
                        @empty
                            <tr>
