...
@section('content')
    <section class="admin-shell-stack">
        ...
        <section class="admin-shell-card">
            <div class="admin-shell-card__body">
                ...
                <form method="POST" action="{{ $formAction }}" class="admin-shell-form admin-shell-stack">
                    @csrf
                    <input type="hidden" name="mode" value="batch-keywords-collapse-spaces">
                    ...
                    <div class="admin-shell-form__field">
                        ...
                        <p class="admin-shell-form__hint">支持换行、逗号和空格混输。保存时会自动去重，只压缩命中商品关键字里的连续空格。</p>
                    </div>
        ...
        <section class="admin-shell-card">
            <div class="admin-shell-card__header">
                <h2>命中商品预览</h2>
                <p class="admin-shell-card__description">先看一眼商品名称、类型、当前状态和当前关键字，再执行连续空格压缩，避免粘错 ID。</p>
            </div>
            <div class="admin-shell-card__body">
