        ...
        <div class="panel-body">
            <div class="detail-grid">
                ...
                <div class="detail-item">
                    ...
                    <div class="detail-value">仅更新支付场景，不改密钥、回调路由和启用状态</div>
                </div>
            </div>
        </div>
    </section>
        ...
        <div class="panel-body">
            <form method="post" action="{{ $formAction }}" class="form-stack">
                ...
                <div class="notice info" style="margin-bottom: 18px;">
                    这张页刻意只收一个低风险动作。我们先把最常见的“统一切换 PC / 手机 / 通用场景”接进后台壳，再逐步扩更多支付侧批量维护能力。
                </div>
                ...
                <div class="filters" style="align-items: stretch;">
                    <label style="grid-column: 1 / -1;">
                        ...
                        <textarea name="ids_text" rows="8" placeholder="例如：&#10;93011&#10;93012&#10;93013">{{ old('ids_text', $defaults['ids_text']) }}</textarea>
                    </label>
                    <label>
                        <span>目标支付场景</span>
                        <small style="display:block; margin-top:6px; color:#66756b;">只切换通道面向的前台场景，不会改支付方式、支付标识或任何密钥字段。</small>
    ...
    @if(($context['requestedCount'] ?? 0) > 0)
        <section class="panel">
            <div class="panel-body">
                ...
                <p class="page-description">先确认当前支付场景、生命周期和状态，再执行批量切换，避免把不该切的通道带进去。</p>
                ...
                @else
                    <div class="notice muted" style="margin-top: 18px;">
                        这次输入的支付通道 ID 里没有匹配到有效记录，提交后不会更新任何支付场景。
                    </div>
                @endif
