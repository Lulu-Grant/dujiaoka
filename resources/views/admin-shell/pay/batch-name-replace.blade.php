...
@section('content')
    ...
    <div class="admin-shell-card">
        <div class="admin-shell-card__header">
            <div>
                <h2 class="admin-shell-card__title">批量替换支付名称片段</h2>
                <p class="admin-shell-card__description">输入支付通道 ID 后，可以先预览命中的通道，再统一替换当前名称中的指定片段。</p>
            </div>
        ...
        <form method="post" action="{{ $formAction }}" class="admin-shell-form">
            ...
            <div class="admin-shell-form__grid">
                ...
                <label class="admin-shell-form__field">
                    ...
                    <input type="text" name="search_text" value="{{ old('search_text', $defaults['search_text']) }}" class="admin-shell-form__input" placeholder="例如：旧活动">
                    <span class="admin-shell-form__hint">只会替换支付名称中完全命中的文本片段。</span>
                    @error('search_text')
                    <span class="admin-shell-form__error">{{ $message }}</span>
                    @enderror
                </label>
                ...
                <label class="admin-shell-form__field">
                    ...
                    <input type="text" name="replace_text" value="{{ old('replace_text', $defaults['replace_text']) }}" class="admin-shell-form__input" placeholder="例如：新活动">
                    <span class="admin-shell-form__hint">留空时会删除命中的名称片段，其他支付配置不会被修改。</span>
                    @error('replace_text')
                    <span class="admin-shell-form__error">{{ $message }}</span>
                    @enderror
                </label>
    ...
    <div class="admin-shell-card">
        <div class="admin-shell-card__header">
            <div>
                ...
                <p class="admin-shell-card__description">预览当前输入 ID 命中的支付通道，确认后再统一替换名称片段。</p>
            </div>
        </div>
