...
@section('content')
    ...
    <div class="admin-shell-card">
        <div class="admin-shell-card__header">
            <div>
                <h2 class="admin-shell-card__title">批量添加支付名称后缀</h2>
                <p class="admin-shell-card__description">输入支付通道 ID 后，可以先预览命中的通道，再统一为当前名称添加后缀。</p>
            </div>
        ...
        <form method="post" action="{{ $formAction }}" class="admin-shell-form">
            ...
            <div class="admin-shell-form__grid">
                ...
                <label class="admin-shell-form__field admin-shell-form__field--full">
                    <span class="admin-shell-form__label">目标后缀</span>
                    <input type="text" name="name_suffix" value="{{ old('name_suffix', $defaults['name_suffix']) }}" class="admin-shell-form__input" placeholder="例如：-春季活动">
                    <span class="admin-shell-form__hint">提交后会把当前支付名称改成“原名称 + 后缀”，适合活动期统一补充尾部标签。</span>
                    @error('name_suffix')
                    <span class="admin-shell-form__error">{{ $message }}</span>
                    @enderror
                </label>
    ...
    <div class="admin-shell-card">
        <div class="admin-shell-card__header">
            <div>
                ...
                <p class="admin-shell-card__description">预览当前输入 ID 命中的支付通道，确认后再统一更新名称后缀。</p>
            </div>
        </div>
