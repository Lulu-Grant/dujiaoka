...
@section('content')
    ...
    <div class="admin-shell-card">
        <div class="admin-shell-card__header">
            <div>
                <h2 class="admin-shell-card__title">批量设置支付名称</h2>
                <p class="admin-shell-card__description">输入支付通道 ID 后，可以先预览命中的通道，再统一修改展示名称。</p>
            </div>
        ...
        <form method="post" action="{{ $formAction }}" class="admin-shell-form">
            ...
            <div class="admin-shell-form__grid">
                ...
                <label class="admin-shell-form__field admin-shell-form__field--full">
                    <span class="admin-shell-form__label">目标支付名称</span>
                    <input type="text" name="pay_name" value="{{ old('pay_name', $defaults['pay_name']) }}" class="admin-shell-form__input" placeholder="例如：活动期支付宝通道">
                    <span class="admin-shell-form__hint">本动作只更新支付名称，用于运营展示，不影响支付标识和回调链路。</span>
                    @error('pay_name')
                    <span class="admin-shell-form__error">{{ $message }}</span>
                    @enderror
                </label>
    ...
    <div class="admin-shell-card">
        <div class="admin-shell-card__header">
            <div>
                ...
                <p class="admin-shell-card__description">预览当前输入 ID 命中的支付通道，确认后再统一修改名称。</p>
            </div>
        </div>
