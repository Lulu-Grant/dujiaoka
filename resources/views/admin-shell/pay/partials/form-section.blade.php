<section class="panel" style="margin-top: 18px;">
    <div class="panel-body">
        <div style="display: flex; justify-content: space-between; gap: 18px; align-items: flex-start; margin-bottom: 16px;">
            <div>
                <div class="page-kicker" style="margin-bottom: 6px;">支付配置分组</div>
            ...
            @if(!empty($section['note']))
                <div style="max-width: 340px; padding: 12px 14px; border-radius: 14px; background: rgba(31, 52, 37, 0.06); color: var(--muted); line-height: 1.7;">
                    {{ $section['note'] }}
                </div>
            @endif
        </div>
        ...
        <div class="filters">
            @foreach($section['fields'] as $field)
                @php
                    ...
                    $currentValue = !empty($field['sensitive'])
                        ? ($field['value'] ?? '')
                ...
                @elseif($fieldType === 'select')
                    <label>
                        ...
                        <select
                            name="{{ $field['name'] }}"
                            @if(!empty($field['required'])) required @endif
                            @if(!empty($field['readonly'])) disabled @endif
                        >
