...
@section('content')
    ...
    <div class="admin-shell-card">
        <div class="admin-shell-card__header">
            <div>
                <h2 class="admin-shell-card__title">批量添加支付名称前缀</h2>
                <p class="admin-shell-card__description">输入支付通道 ID 后，可以先预览命中的通道，再统一为当前名称添加前缀。</p>
            </div>
        ...
        <form method="post" action="{{ $formAction }}" class="admin-shell-form">
            ...
            <div class="admin-shell-form__grid">
                ...
                <label class="admin-shell-form__field admin-shell-form__field--full">
                    ...
                    <input type="text" name="name_prefix" value="{{ old('name_prefix', $defaults['name_prefix']) }}" class="admin-shell-form__input" placeholder="例如：[活动期]">
                    <span class="admin-shell-form__hint">提交后会把当前支付名称改成“前缀 + 原名称”，适合活动期统一标记。</span>
                    @error('name_prefix')
                    <span class="admin-shell-form__error">{{ $message }}</span>
                    @enderror
                </label>
    ...
    <div class="admin-shell-card">
        <div class="admin-shell-card__header">
            <div>
                ...
                <p class="admin-shell-card__description">预览当前输入 ID 命中的支付通道，确认后再统一更新名称前缀。</p>
            </div>
        </div>
