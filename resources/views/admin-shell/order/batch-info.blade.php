...
@section('content')
    ...
    <div class="admin-shell-card">
        <div class="admin-shell-card__header">
            <div>
                <h2 class="admin-shell-card__title">批量设置订单附加信息</h2>
                <p class="admin-shell-card__description">输入订单 ID 后，可以先预览命中的订单，再统一更新附加信息。</p>
            </div>
        ...
        <form method="post" action="{{ $formAction }}" class="admin-shell-form">
            ...
            <div class="admin-shell-form__grid">
                ...
                <label class="admin-shell-form__field admin-shell-form__field--full">
                    <span class="admin-shell-form__label">目标附加信息</span>
                    <textarea name="info" rows="5" class="admin-shell-form__textarea" placeholder="例如：2026 春季活动人工复核">{{ old('info', $defaults['info']) }}</textarea>
                    <span class="admin-shell-form__hint">本动作只更新订单附加信息，适合统一补充运营备注、售后说明或人工核对标记。</span>
                    @error('info')
                    <span class="admin-shell-form__error">{{ $message }}</span>
                    @enderror
                </label>
    ...
    <div class="admin-shell-card">
        <div class="admin-shell-card__header">
            <div>
                ...
                <p class="admin-shell-card__description">预览当前输入 ID 命中的订单，确认后再统一写入附加信息。</p>
            </div>
        </div>
        ...
        <div class="admin-shell-table-wrap">
            <table class="admin-shell-table">
                ...
                @empty
                    <tr>
                        <td colspan="8" class="admin-shell-table__empty">当前没有匹配到任何订单，先输入有效 ID 再继续。</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
