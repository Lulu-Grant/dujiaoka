...
@section('content')
    ...
    <div class="admin-shell-card">
        <div class="admin-shell-card__header">
            <div>
                <h2 class="admin-shell-card__title">批量添加订单标题前缀</h2>
                <p class="admin-shell-card__description">输入订单 ID 后，可以先预览命中的订单，再统一为当前订单标题添加前缀。</p>
            </div>
        ...
        <form method="post" action="{{ $formAction }}" class="admin-shell-form">
            ...
            <div class="admin-shell-form__grid">
                ...
                <label class="admin-shell-form__field admin-shell-form__field--full">
                    <span class="admin-shell-form__label">目标标题前缀</span>
                    <input type="text" name="title_prefix" value="{{ old('title_prefix', $defaults['title_prefix']) }}" class="admin-shell-form__input" placeholder="例如：[人工复核] ">
                    <span class="admin-shell-form__hint">提交后会把当前标题改成“前缀 + 原标题”，适合活动标记、售后复核或历史订单整理。</span>
                    @error('title_prefix')
                    <span class="admin-shell-form__error">{{ $message }}</span>
                    @enderror
                </label>
    ...
    <div class="admin-shell-card">
        <div class="admin-shell-card__header">
            <div>
                ...
                <p class="admin-shell-card__description">预览当前输入 ID 命中的订单，确认后再统一添加标题前缀。</p>
            </div>
        </div>
