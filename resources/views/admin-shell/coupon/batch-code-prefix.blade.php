...
@section('content')
    ...
    <div class="admin-shell-card">
        <div class="admin-shell-card__header">
            <div>
                <h2 class="admin-shell-card__title">批量添加优惠码前缀</h2>
                <p class="admin-shell-card__description">输入优惠码 ID 后，可以先预览命中的优惠码，再统一为当前优惠码内容添加前缀。</p>
            </div>
        ...
        <form method="post" action="{{ $formAction }}" class="admin-shell-form">
            ...
            <div class="admin-shell-form__grid">
                ...
                <label class="admin-shell-form__field admin-shell-form__field--full">
                    ...
                    <input type="text" name="prefix" value="{{ old('prefix', $defaults['prefix']) }}" class="admin-shell-form__input" placeholder="例如：SPRING-">
                    <span class="admin-shell-form__hint">提交后会把当前优惠码改成“前缀 + 原优惠码”，适合活动期统一标记渠道批次。</span>
                    @error('prefix')
                    <span class="admin-shell-form__error">{{ $message }}</span>
                    @enderror
                </label>
    ...
    <div class="admin-shell-card">
        <div class="admin-shell-card__header">
            <div>
                ...
                <p class="admin-shell-card__description">预览当前输入 ID 命中的优惠码，确认后再统一更新前缀。</p>
            </div>
        </div>
