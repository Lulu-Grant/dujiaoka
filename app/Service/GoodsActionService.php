...
class GoodsActionService
{
    ...
    public function batchDescriptionContext(array $goodsIds): array
    {
        $goods = Goods::query()
            ->whereIn('id', $goodsIds)
            ->orderBy('id')
            ->get(['id', 'gd_name', 'type', 'is_open', 'description']);
    ...
    public function updateDescription(array $goodsIds, string $description): int
    {
        if (empty($goodsIds)) {
            return 0;
        }
