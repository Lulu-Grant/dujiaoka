...
最新预发布版本为 `v3.0.0-beta.2`，发布分支为 `codex/laravel7-bridge-experiment`。这一版已经完成 Laravel 7.30.7 桥接实验验收，保留 PHP 7.4 兼容验证，并增加 PHP 8.1 Docker 验证链路。
...
- 当前默认品牌已统一为“独角数卡西瓜版”。
- 最新预发布 `v3.0.0-beta.2` 已完成：Laravel 7.30.7 桥接实验、PHP 7.4 / PHP 8.1 双运行时 PHPUnit、后台 smoke、安装流程现代化、后台壳主承载和支付通道裁剪。
- 当前路线以“默认主线保持可验证 + 预发布分支承载升级实验 + 后台壳主承载”为原则推进。
...
- 在 `codex/laravel7-bridge-experiment` 发布分支将 Laravel 桥接到 `7.30.7`，并固定 Composer 兼容约束，保持 PHP 7.4 / PHP 8.1 双运行时测试可通过
...
- 最新预发布状态：`v3.0.0-beta.2`
- 升级实验状态：Laravel `7.30.7` 桥接实验已通过，暂不直接跳 Laravel 8/10
...
- 最新预发布 `v3.0.0-beta.2` 已具备：Laravel 7.30.7 桥接实验、PHP 7.4 / PHP 8.1 双运行时验证、本地快速拉站、GitHub Actions 自动回归、安装现代化主路径和后台壳主承载基础。
